...
#----------------------------------------------------------------------------------------------------------------------#
    ...
    #    Что делает: Берёт первую цифру строки и преобразует её обратно в целое число.
    # Пример: Для строки '40203' первая цифра — это '4', которая становится числом 4.
#----------------------------------------------------------------------------------------------------------------------#
...
#----------------------------------------------------------------------------------------------------------------------#
    ...
    if len(str_number) > 1:
        #    Что делает: Проверяет, состоит ли строка из более чем одной цифры.
        #Цель: Если есть ещё цифры, продолжаем рекурсивный процесс.
#----------------------------------------------------------------------------------------------------------------------#
    #Шаг 4: Рекурсивное перемножение цифр
        return first * get_multiplied_digits(int(str_number[1:]))
    #Что делает:
    #    Умножает текущую цифру (first) на результат рекурсивного вызова функции для оставшейся части строки (str_number[1:]).
    #    int(str_number[1:]) преобразует оставшуюся часть строки обратно в число.
    #Пример: Для числа 40203:
    #    first = 4, а оставшаяся часть — '0203'.
    #    Следующий вызов функции: 4 * get_multiplied_digits(203)
#----------------------------------------------------------------------------------------------------------------------#
#   # Шаг 5: Обработка последней цифры
    # Что делает: Если осталась только одна ненулевая цифра, возвращает её значение.
    # Пример: Если осталась цифра 3, функция вернёт 3.
    elif first != 0:
        return first
#----------------------------------------------------------------------------------------------------------------------#
    #Шаг 7: Обработка нулей
    else:
        return 1
    #    Что делает: Если текущая цифра — 0, возвращает 1. Это необходимо, чтобы ноль не обнулял произведение.
    #Почему это важно: Произведение любого числа на 0 равно 0, но в данной задаче нужно игнорировать нули,
    # поэтому они заменяются на 1.
#----------------------------------------------------------------------------------------------------------------------#
...
#----------------------------------------------------------------------------------------------------------------------#
# Основная функция: Ф-ия подсчета суммы эелементов
def calculate_structure_sum(*args):
    total = 0
# Функция принимает переменное количество аргументов (*args). Это позволяет передавать как одиночные элементы,
# так и целые структуры (списки, словари и т.д.).
#----------------------------------------------------------------------------------------------------------------------#
    for i in args:
# Функция перебирает каждый элемент из аргументов (args). Для каждого элемента выполняется проверка типа данных.
#----------------------------------------------------------------------------------------------------------------------#
        if isinstance(i, (int, float)):
            total += i
#    Что делает: Если элемент является числом (int или float), добавляет его значение к общей сумме.
                #Пример: Если элемент 4, он добавляется к total.
#----------------------------------------------------------------------------------------------------------------------#
        elif isinstance(i, str):
            total += len(i)
#    Что делает: Если элемент — строка, добавляется длина строки.
        #Пример: "Hello" имеет длину 5, поэтому к total добавляется 5.
#----------------------------------------------------------------------------------------------------------------------#
        elif isinstance(i, (list, tuple, set)):
            total += calculate_structure_sum(*i)
#       Что делает: Если элемент является коллекцией (списком, кортежем или множеством), функция вызывается рекурсивно
        #           для каждого элемента внутри этой коллекции.
        #Рекурсия: Раскрывает содержимое коллекции и обрабатывает каждый элемент.
        #Пример: Для [1, 2, 3] вызовется calculate_structure_sum(1, 2, 3).
#----------------------------------------------------------------------------------------------------------------------#
        elif isinstance(i, dict):
            total += calculate_structure_sum(*i.keys(), *i.values())
#    Что делает: Если элемент — словарь, функция вызывается для всех ключей и значений словаря.
   # Почему обрабатываются ключи и значения: Ключи могут быть строками, а значения — числами или другими структурами.
   # Пример: Для словаря {'a': 4, 'b': 5}:
   #    Ключи: 'a' и 'b' (их длины 1 каждая).
   #    Значения: 4 и 5.
   #    Результат: 1 (a) + 4 + 1 (b) + 5 = 11.
#----------------------------------------------------------------------------------------------------------------------#
    return total
    #После обработки всех элементов возвращается сумма.
#----------------------------------------------------------------------------------------------------------------------#
# вывод результата
